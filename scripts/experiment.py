...
from qaoa_study.experiments import (
    digest, freeze_references, load_attempts, read_batch, read_references,
    run_worker, save_batch, save_references, select_attempts, source_identity,
)
from qaoa_study.records import read_json, write_once_json
...
def summarize_batch(batch, attempts_directory, references_directory):
    """Reconstruct one graph at a time, including costs of excluded attempts."""
    ...
    attempt_root = Path(attempts_directory)
    files = sorted(attempt_root.glob("*.json"))
    lookup = {task["task_id"]: task for task in tasks}
    task_groups, file_groups = defaultdict(list), defaultdict(list)
    for task in tasks:
        task_groups[task["iso_class_id"]].append(task)
    for path in files:
        if path.name == "manifest.json":
            continue
        task_id = path.name.partition("--")[0]
        if task_id not in lookup:
            raise ValueError("Attempt directory includes an unplanned task.")
        file_groups[lookup[task_id]["iso_class_id"]].append(path)
    summary = build_summary(library, [], {}, {}, manifest["config"].get("analysis"))
    physical_cost, selected_ids, completed_ids, audit = {}, {}, set(), {}
    execution = None
    for graph in sorted(library["graphs"], key=lambda row: row["iso_class_id"]):
        graph_tasks = task_groups[graph["iso_class_id"]]
        attempts, execution = load_attempts(manifest, graph_tasks, attempt_root,
                                             paths=file_groups[graph["iso_class_id"]], audit=audit, execution=execution)
        selected = select_attempts(attempts)
        part = build_summary({**library, "graphs": [graph]}, graph_tasks, selected, references,
                             manifest["config"].get("analysis"))
        for key in ("graph_qaoa", "restarts", "diagnostics"):
            summary[key].extend(part[key])
        selected_ids.update({key: row["attempt_id"] for key, row in selected.items()})
        for row in attempts:
            ...
            costs = physical_cost.setdefault(role, {
    ...
    for path in files:
        checksum = audit.get("file_hashes", {}).get(path.name)
        if checksum is None:
            checksum = hashlib.sha256(path.read_bytes()).hexdigest()
        input_digest.update((path.name + "\0" + checksum + "\n").encode())
    summary.update(batch_id=manifest["batch_id"], execution_id=execution["execution_id"] if execution else None,
                   analysis_source=source_identity(), coverage=manifest["coverage"],
                   attempt_selection_rule=manifest["attempt_selection_rule"], all_attempt_costs_by_role=physical_cost,
                   ...
                   selected_attempts=selected_ids, attempt_input_files=len(files),
                   attempt_input_digest=input_digest.hexdigest(),
                   input_digest_rule="SHA256 of sorted basename + NUL + SHA256(file) + newline")
    return summary
